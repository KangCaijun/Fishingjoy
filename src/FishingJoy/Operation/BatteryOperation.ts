...
module operation {
	export class BatteryOperation extends BaseOperation {
		...
		private _rotationSeat(): void {
			...
			let fishLayer: egret.DisplayObjectContainer = manager.LayerManager.instance.getLayer(LAYER.Fish);
		...
		private _initPoint(): void {
			...
			manager.LayerManager.instance.addToLayer(this._aimBig, LAYER.AIM);
			...
			manager.LayerManager.instance.addToLayer(pointImg, LAYER.AIM);
		...
		private _aimFish(rev: game.SeatData): void {
			...
			if (rev.lock) {
				if (Master.instance.uid == uid) {
					...
					EventManager.fireEvent(EVENT_ID.PLAY_FIRE_EFF, [true, SHOOT_TYPE.AIM]);
				}
				...
				if (Master.instance.uid == uid) {
					...
					EventManager.fireEvent(EVENT_ID.PLAY_FIRE_EFF, [false, SHOOT_TYPE.AIM])
				}
				this._gameObj.isAimShoot = false;
				this._clearAimEff();
				Laya.timer.clear(this, this._drawAimEff)
		...
		public unregister(): void {
			...
			manager.LayerManager.instance.removeFromLayer(this._smallPoint, LAYER.AIM);
			manager.LayerManager.instance.removeFromLayer(this._aimBig, LAYER.AIM);
