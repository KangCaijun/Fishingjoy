...
class Master {
	...
	private _mainSeatChanged() {
		...
		if (seatId > 2) {
			...
			let rotation = 180;
			manager.LayerManager.instance.setLayerRotation(rotation, LAYER.Fish);
			manager.LayerManager.instance.setLayerRotation(rotation, LAYER.FishingNet);
			manager.LayerManager.instance.setLayerRotation(rotation, LAYER.Bullet);
